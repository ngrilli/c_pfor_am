...
**
     ...
     + nTwinStart,nTwinEnd)
      ...
      real*8, intent(in) :: dtime, signtau(nSys)
      real*8, intent(in) :: signtautwin(nTwin)
      real*8, intent(in) :: gndcut(nSys)
      real*8, intent(in) :: gndtot,rhossd
      real*8, intent(in) :: xNorm(nSys,3)
	  real*8, intent(in) :: xDir(nSys,3)
	  real*8, intent(in) :: tau(nSys)
	  real*8, intent(in) :: tauc(nSys)
	  real*8, intent(in) :: burgerv(nSys)
      real*8, intent(in) :: xTwinNorm(nTwin,3)
	  real*8, intent(in) :: xTwinDir(nTwin,3)
	  real*8, intent(in) :: tautwin(nTwin)
	  real*8, intent(in) :: tauctwin(nTwin)
      real*8, intent(in) :: twinvolfrac(nTwin)
	  real*8, intent(in) :: twinvolfractotal ! twin volume fraction
...
C
      ...
      if ((xtau >= 0.5 .or. twinvolfrac(I) > 0.5) 
     + .and. twinvolfrac(I) < 1.0) THEN

      gammaTwinDot(I)=gamma0dottwin*
     + (xtau**nsliprate)*signtautwin(I) ! signtautwin always positive
      ...
      gammaTwinDot(I)=gammaTwinDot(I) +
     + 0.299*(1.0-twinvolfrac(I))
              end if
              ...
              ! such that 4,5,6 components contributes as (b_i n_j + b_j n_i)
      result4 = result4 + 
     + (gamma0dottwin*nsliprate/tauctwin(I))*
     + dtime*result1*SNNS
