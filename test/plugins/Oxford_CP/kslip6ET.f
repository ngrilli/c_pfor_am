      ...
      subroutine kslip6ET(xNorm,xDir,tau,
     + signtau,
     + tauc,burgerv,dtime,
     + nSys,iphase,irradiate,
     + gndcut,gndtot,rhossd,
     + Lp,tmat,gammaDot)
!Slip Rule with constant coefficients for simplicity
     ...
     + irradiate
      ...
      real*8, intent(in) ::	gndcut(nSys), gndtot,rhossd
      real*8, intent(in) :: xNorm(nSys,3),xDir(nSys,3)
      real*8, intent(in) :: tau(nSys),tauc(nSys),burgerv(nSys)

      real*8, intent(out) :: Lp(3,3),tmat(6,6), gammaDot(nSys)
      
      integer :: i
      real*8  :: alpha,beta,result1, rhom,dF,
     + f,T,k,gamma0,b,psi,V,
