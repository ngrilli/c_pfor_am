...
********************************************
      ...
      SUBROUTINE kmat(dtime,nsvars,usvars,xI,
     + jelem,kint,time,F,
     + L,iphase,irradiate,C,stressvec,
...
** The following parameters must be set **
      ...
      real*8, parameter :: Temperature = 1173.5
...
C     *** COMPUTE TRIAL STRESS ***
      ...
      trialstress = stressvec +
     + matmul(xStiffdef,dtotstran) - 
...
C     *** USE NEWTON METHOD TO DETERMINE STRESS INCREMENT ***
      ...
      CALL kslip0(xNorm,xDir,tau,tauc,
     + caratio,dtime,nSys,0.0,
     + iphase,Lp,tmat) 
     
      ELSE IF (kslip == 5) THEN ! Original slip rule with GND coupling 
                 
      CALL kslip5ET(xNorm,xDir,tau,
     + signtau,tauc,burgerv,rhossd,
     + gndtot,gndall,gndcut,gndmob,
     + dtime,nSys,ns,iphase,Lp,tmat)

      ELSE IF (kslip == 6) THEN ! updated slip rule with GND coupling
      
      CALL  kslip6ET(xNorm,xDir,tau,
     + signtau,tauc,burgerv,dtime,
     + nSys,iphase,irradiate,gndcut,
     + gndtot,rhossd,Lp,tmat,gammaDot)
...
C     *** DETERMINE DENSITY OF GNDs
    ...
    ! Edge-screw separated
      ...
      CALL kgndl2ET(curlfp,xNorm,xDir,
     + tau,burgerv,iphase,nSys,ns,
     + screwplanes,jelem,kint,time,
     + gndall,gndcut,gndmob)
...
C     *** stress tensor
      ...
      write(*,*) "WARNING gmatinv not updated at:", 
     + jelem,kint, kinc
      END IF      
