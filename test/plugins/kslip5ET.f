!Slip Rule Re-development
      subroutine kslip5ET(xNorm,xDir,tau,signtau,tauc,
     +     burgerv,rhossd,gndtot,
     +     gndall,gndcut,gndmob,dtime,ne,ns,iphase,Lp,tmat)
      ...
      real*8, intent(in) :: rhossd,gndtot,dtime, gndall(ne+ns) 
	  real*8, intent(in) :: gndcut(ne), gndmob(ne), signtau(ne)
      real*8, intent(in) :: xNorm(ne,3),xDir(ne,3)
	  real*8, intent(in) :: tau(ne),tauc(ne),burgerv(ne)
