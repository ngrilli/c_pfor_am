...
!Use this to avoid editing multiple common blocks independently
		...
		integer, parameter :: NUpDown = 1
		! NUpDown * nElements * nintpts
        integer, parameter :: ArrayNUpDown = 8
...
! loop over all elements and IPs
     ...
     + dot_product(NeighbourConnection(1:3),NeighbourConnection(1:3))
      ...
      NeighbourProjection = 
     + abs(dot_product(NeighbourConnection(1:3),ttwinnor(1:3)))
      ...
      if (NeighbourDistCyl < 1.001 
     + .and. NeighbourProjection < 5.001) then ! up/down
        NeighbourUpDownIndex(twinindex) = 
     + NeighbourUpDownIndex(twinindex) + 1
        if (NeighbourUpDownIndex(twinindex) <= NUpDown) then
      ArrayIndexUpDown = (noel-1)*nintpts*NUpDown + 
     + (npt-1)*NUpDown+NeighbourUpDownIndex(twinindex)
