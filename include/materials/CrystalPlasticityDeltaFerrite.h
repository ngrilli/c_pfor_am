...
class CrystalPlasticityDeltaFerrite;
...
class CrystalPlasticityDeltaFerrite : public CrystalPlasticityDislocationUpdateBase
{
public:
  static InputParameters validParams();

  CrystalPlasticityDeltaFerrite(const InputParameters & parameters);
...
protected:
  ...
  const Real _static_recovery_A; // coefficient (s^-1)
  const Real _static_recovery_Q; // Activation energy (J/mol)
  const Real _gas_constant_R;    // gas constant (8.314 J/mol K)
