...
class CrystalPlasticityHCP : public CrystalPlasticityDislocationUpdateBase
{
public:
  static InputParameters validParams();

  CrystalPlasticityHCP(const InputParameters & parameters);
...
protected:
  ...
  // Evolution of the dislocation density
  ...
  const Real _burgers_vector_mag_1;
  const Real _burgers_vector_mag_2;
  std::vector<Real> _burgers_vector_vec;
  ...
  // const Real _r;
  const Real _a_self;
  const Real _a_latent;
  
  // Peierls stress
  const Real _tau_c_0_scaling;
  const Real _tau_c_0_basal;
  const Real _tau_c_0_pris;
  const Real _tau_c_0_pyra;
  const Real _tau_c_0_1stpy;
  const Real _tau_c_0_2ndpy;
